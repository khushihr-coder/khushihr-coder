<p align="center">
  <img src="github-header-banner.png" alt="Khushi Rathod Banner" />
</p>

# Hi, I'm Khushi Rathod 👋
### Computer Engineering Student | Web Developer | AI/ML Projects & Startup Development

🚀 Building web applications, startup products, and digital experiences

🤖 Exploring AI/ML, Computer Vision, and Assistive Technologies

🛰️ Contributing to CubeSat, Space Technology, and Robotics Projects

♿ Developing accessibility-focused solutions like MotionQ

🎮 Creating interactive software experiences through projects like Code of Silence

## Featured Projects

### Zirccle
Designed and developed a startup website in collaboration with founders, including responsive UI, deployment support, branding assets, and Linktree integration.
🔗 https://zirccle.vercel.app/

### MotionQ
Gesture-based human-computer interaction system enabling hands-free computer control through facial gesture recognition and blink detection.
🔗 https://motionvq.netlify.app/

### Code of Silence
Interactive 3D mystery game built using React.js and Vite.
🔗 https://murder-mystery-phi.vercel.app/

### SpaceScope
Interactive space exploration platform featuring NASA API integration and real-time planetary visualization.
🔗 https://spacescope-nine.vercel.app/

## Connect With Me

LinkedIn:
https://www.linkedin.com/in/khushi-rathod-13b913348/

GitHub:
https://github.com/khushihr-coder

Email:
rathodkhu23@gmail.com
